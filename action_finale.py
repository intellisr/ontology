...
def upload_dict_as_document(no,action):
  doc2 = collection.find_one({'no':no})
  if "action3" in doc2:
      print('Action Exsist')
  else:
    collection.update_one(
          {"no": no},  # Filter to find the document by "no"
          {"$set": {"action3": action}}  # Update operation
    )
...
for i in range(1,1114):
  if i not in [543,666,969,1009,1107]:
    ...
    doc2 = collection.find_one({'no':i})
    if "action3" in doc2:
      print('already have')
    else:
      print(i)  
      docs = collection.find(
      { "$text": { "$search": pre_answer } },
      ...
      for doc in docs:
        if first==0:
          ...
          if doc['score'] > first-3:
              no_list.append(doc)

      for c_one in no_list:
        upload_dict_as_document(c_one['no'],"action_"+str(firstN)) 